...
def get_dataset(n_way, n_support, n_query, dataset='swissprot'):
    # Since RelationNet is meta based approach
    if dataset == 'swissprot':
        train_dataset = SPSetDataset(n_way=n_way, n_support=n_support, n_query=n_query, n_episode=100, mode='train')
        val_dataset = SPSetDataset(n_way=n_way, n_support=n_support, n_query=n_query, mode='val')
    else:
        train_dataset = TMSetDataset(n_way=n_way, n_support=n_support, n_query=n_query, n_episode=100, mode='train')
            ...
            for bb_lyr in backbone_layers:
                for bb_rel_lyr in backbone_relation_layers:
                    for hd in hidden_dim:
                        average_mean_acc = 0.0
                        # For one fold take train and validation splits
                        ...
                        # If we have better average mean accuracy 
